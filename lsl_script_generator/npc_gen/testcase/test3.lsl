...
list I_S0_T = [1, 3, 15, 0, 1, "", 0, 1, "", 1, "", 1, "", ""];
list I_S1_T = [1, 3, 15, 0, 1, "", 0, 1, "", 1, "", 1, "", ""];
//helper functions
reset_all() 
{  // resets all globals  
    llSetRot(myrotation);
...
goto_idle_state()
{
  ...
  else if (llSubStringIndex(state_name, "_S0") != -1) state Idle_S0;
  else if (llSubStringIndex(state_name, "_S1") != -1) state Idle_S1;
...
response_exit_state()
{
  ...
  else if (state_name == "Respond_S0") state Respond1_S0;
  else if (state_name == "Respond1_S0") state Respond_S0;
  else if (state_name == "Respond_S1") state Respond1_S1;
  else if (state_name == "Respond1_S1") state Respond_S1;
...
process_state_specific_msg_S0(integer c, string n, key ID, string msg)
{
  integer ii;
  if (c == npc_state_control_channel) {
    if (msg == "-three1") {
      current_interaction = I_S0_A0;
      curr_int_index = do_ask_action(3);
      ii = llList2Integer(current_interaction, curr_int_index);
      if (ii == 0) state Idle_S0;
      else state Respond_S0;
    }
    else osNpcSay(npc, "Unknown command in state control channel.");
  }
}

process_state_specific_msg_S1(integer c, string n, key ID, string msg)
{
  integer ii;
  if (c == npc_state_control_channel) {
    if (msg == "-three1") {
      current_interaction = I_S1_A0;
      curr_int_index = do_ask_action(3);
      ii = llList2Integer(current_interaction, curr_int_index);
      if (ii == 0) state Idle_S1;
      else state Respond_S1;
    }
    else osNpcSay(npc, "Unknown command in state control channel.");
  }
}
...
process_state_specific_control_msg(integer c, string n, key ID, string msg)
{
  ...
  else if (llSubStringIndex(state_name, "_S0") != -1)
    process_state_specific_msg_S0(c, n, ID, msg);
  else if (llSubStringIndex(state_name, "_S1") != -1)
    process_state_specific_msg_S1(c, n, ID, msg);
}
...
state Ask_S0
{
    ...
    listen(integer c, string n, key ID, string msg)
    {
		...
		if (debug_level >= 10) 
		  osNpcSay(npc, "XXX" + ii);
                if (ii == 0) {state Idle_S0; llSetTimerEvent(15);}
                else state Respond_S0;
            }
        }  else process_common_listen_port_msg(c, n, ID, msg);   
    }
...
state Ask_S1
{
    ...
    listen(integer c, string n, key ID, string msg)
    {
		...
		if (debug_level >= 10) 
		  osNpcSay(npc, "XXX" + ii);
                if (ii == 0) {state Idle_S1; llSetTimerEvent(15);}
                else state Respond_S1;
            }
        }  else process_common_listen_port_msg(c, n, ID, msg);   
    }
