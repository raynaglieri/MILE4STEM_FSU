...
list I_default_A0 = [1, 3, 51,
0, 1, "yes", 0, 1, "", 1, "", 1, "", "",
2, 19, 36, 37, 51,
0, 3, 1, "pay attention", "concentrate", "focus",
0, 1, "I just got distracted", -1, 1, "-goto:notbored", 1, "", 1, "", "", 
0,
0, 0, 0, 0, 1, "whatever", 0, 1, "", 1, "", 1, "", "", 
0];
list I_default_T = [1, 3, 15,
0, 1, "", 0, 1, "", 1, "", 2, "avatar_express_wink", "avatar_no_unhappy", "", 
0];
list I_notbored_T = [1, 3, 14,
0, 1, "I am not bored.", 0, 1, "", 1, "", 1, "", "", 
0];
//helper functions
reset_all() 
{  // resets all globals  
...
goto_idle_state()
{
  ...
  else if (llSubStringIndex(state_name, "_notbored") != -1) state Idle_notbored;
...
response_exit_state()
{
  ...
  else if (state_name == "Respond_notbored") state Respond1_notbored;
  else if (state_name == "Respond1_notbored") state Respond_notbored;
...
process_state_specific_msg_notbored(integer c, string n, key ID, string msg)
{
  integer ii;
}
...
process_state_specific_control_msg(integer c, string n, key ID, string msg)
{
  ...
  else if (llSubStringIndex(state_name, "_notbored") != -1)
    process_state_specific_msg_notbored(c, n, ID, msg);
}
...
state Idle_notbored
{
    state_entry()
     {
        if(debug_level)
            llSay(0,"Idle");
        state_name = "Idle_notbored";
	current_interaction = I_notbored_T;
	do_ask_action(3);
        register_common_channel_timer(random_integer(30, 30));
    }    
	...
	current_interaction = I_notbored_T;
	do_ask_action(3);
        llSetTimerEvent(random_integer(30, 30));
    }  
...
state Ask_notbored
{
    ...
    timer()
    {
        ask_question();
        localcount++;
        if (localcount >=3) state Idle_notbored;
        else llSetTimerEvent(local_timer);
    }
    ...
    listen(integer c, string n, key ID, string msg)
    {
		...
		if (debug_level >= 10) 
		  osNpcSay(npc, "XXX" + ii);
                if (ii == 0) {state Idle_notbored; llSetTimerEvent(15);}
                else state Respond_notbored;
            }
        }  else process_common_listen_port_msg(c, n, ID, msg);   
    }
} 
...
state Respond_notbored
{
    state_entry()
    {
        //ask_question();
        state_name = "Respond_notbored";
        curr_action_index = next_action_index;
        localcount = 0; saved_curr_int_index = curr_int_index;
        if (debug_level) osNpcSay(npc, "respond state");
        local_timer = 30;
...
state Respond1_notbored
{
    state_entry()
    {
        state_name = "Respond1_notbored";
        localcount = 0; saved_curr_int_index = curr_int_index;
        curr_action_index = next_action_index;
        local_timer = 30;
        if (debug_level) osNpcSay(npc, "respond1 state");
